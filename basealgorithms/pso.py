...
class Particle(object):
    def __init__(self, function, dim, position=None, factor=None, global_solution=None, lbest_pos=None, factorid=None):
        self.f = function
        ...
        if position is None:
            ...
            self.lbest_position = np.array(self.position)
        elif position is not None:
            self.position = position
        self.set_fitness(self.calculate_fitness(global_solution))

        if lbest_pos is not None:
    ...
    def set_fitness(self, fit):
        ...
        if fit < self.lbest_fitness:
            ...
            self.lbest_position = np.copy(self.position)

    def set_position(self, position):
        self.position = np.array(position)

    def set_solution(self, solution):
        self.set_position(solution[self.factor])

    def get_position(self):
        return np.clip(self.position, self.f.lbound, self.f.ubound)

    def update_individual_after_compete(self, global_solution=None,clip=False):
        ''' For compete in FEA, global_solution is updated '''
        solution = self.calculate_solution(global_solution,clip=clip)
        #self.set_solution(solution) # This wouldn't change the position
        self.set_fitness(self.calculate_fitness(solution=solution,clip=clip))
    
    def calculate_solution(self, glob_solution=None, position=None, clip=True):
        '''
        :param glob_solution: context vector
        :param position: factor values only
        :return: Unset new `solution = glob_solution[factor] <- position`
        '''
        if position is None:
        ...
        solution = np.copy(glob_solution)
    ...
    def calculate_fitness(self, glob_solution=None, position=None, solution=None, clip=True):
        '''
        :param glob_solution: context vector
        :param position: factor values only
        :param solution: full solution
        '''
        if solution is None:
            solution = self.calculate_solution(glob_solution,position)
        if clip:
            solution = np.clip(solution, self.f.lbound, self.f.ubound)
    ...
    def update_particle(self, omega, phi, global_best_position, v_max, global_solution=None):
        self.update_velocity_step(omega, phi, global_best_position, v_max)
        self.update_position_step(global_solution)

    def update_velocity_step(self, omega, phi, global_best_position, v_max):
        # velocity = [x for x in self.velocity]
        n = self.dim

        inertia = np.multiply(omega, self.velocity)
        phi_1 = phi*np.random.random(n)  # exploration
        personal_exploitation = self.lbest_position - self.position  # exploitation
        personal = phi_1 * personal_exploitation

        phi_2 = phi*np.random.random(n)  # exploration
        ...
        self.velocity = np.clip(new_velocity,-v_max,v_max)
        # self.velocity = np.array([self.clamp_value(v, -v_max, v_max) for v in new_velocity])

    def update_position_step(self, global_solution=None):
        ...
        # self.position = position
        self.set_fitness(self.calculate_fitness(global_solution,clip=False))
...
class PSO(object):
    def __init__(self, generations, population_size, function, dim, factor=None, global_solution=None, factorid=None, omega=0.729, phi=1.49618):
        ...
        self.pop = [Particle(function, dim, factor=factor, global_solution=global_solution,factorid=factorid) for x in range(population_size)]
    ...
    def find_current_best(self):
        ...
        return Particle(self.f, self.dim, position=sorted_[0].position, factor=self.factor,
                 global_solution=self.global_solution, lbest_pos=sorted_[0].lbest_position,factorid=sorted_[0].factorid)
    ...
    def update_best(self,candidate=None):
        if candidate is None:
            candidate = self.find_current_best()
            self.pbest_history.append(candidate)
        if candidate.fitness < self.gbest.fitness:
            self.gbest = candidate

    def set_global_solution(self,global_solution):
        self.global_solution = global_solution
    ...
    def replace_worst_solution(self, global_solution):
        ...
        worst = self.pop[-1]
        worst.set_solution(global_solution)
        worst.set_fitness(worst.calculate_fitness(global_solution))
        self.update_best(candidate=worst)

        # if fit < self.pop[0].fitness:
        #     assert(np.isclose(fit,self.gbest.fitness))
...
if __name__ == '__main__':
    ...
    # print(pso.gbest.position,pso.gbest.fitness,sep="\n",end="\n\n")
    # print("===")


    # from pyswarms.single.global_best import GlobalBestPSO
    # bounds = (f.lbound*np.ones(dim),f.ubound*np.ones(dim))
    # options = {'c1': 1.49618, 'c2': 1.49618, 'w': 0.729}
    # optimizer = GlobalBestPSO(n_particles=popsize, dimensions=dim, options=options, bounds=bounds)
    # cost, pos = optimizer.optimize(f.run, iters=gens*100)
    # print(f.run(pos),pos,sep="\n",end="\n\n")
